...
class SQLQuery(BaseModel):
    sql_query: str = Field(description="A SQL query to execute on the database")
...
def sql_inter(sql_query: str) -> str:
    ...
    connection = pymysql.connect(host=host, user=user, password=mysql_pw, database=db, port=port)
...
def fig_inter(py_code: str, fname: str) -> str:
    ...
    # ✅ 设置图像保存路径（你自己的绝对路径）
    base_dir = r"/Users/baichen/git/agent-chat-ui/public"
...
tools = [search_tool, sql_inter, extract_data, python_inter, fig_inter, human_assistance]
